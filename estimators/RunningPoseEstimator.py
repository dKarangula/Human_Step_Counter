...
# Need to flip positions because from left to right the model outputs inverted values
if leftToRightRun:
    LEFT_HIP = 23
    RIGHT_HIP = 24
...
def noseDetected(detection_result: PoseLandmarkerResult):
  pose_landmarks_list = detection_result.pose_landmarks

  if len(pose_landmarks_list) == 1:
    if pose_landmarks_list[0][NOSE].presence > 0.5:
      return True
  return False

def alignedLeftFoot(detection_result: PoseLandmarkerResult):
  pose_landmarks_list = detection_result.pose_landmarks
  alignedLeft = withinTolerance(pose_landmarks_list[0][LEFT_HIP].x, [pose_landmarks_list[0][LEFT_KNEE].x, pose_landmarks_list[0][LEFT_HEEL].x], [0.06, 0.05]) # 0.06, 0.06 for running
  return alignedLeft

def alignedRightFoot(detection_result: PoseLandmarkerResult):
  pose_landmarks_list = detection_result.pose_landmarks
  alignedRight = withinTolerance(pose_landmarks_list[0][RIGHT_HIP].x, [pose_landmarks_list[0][RIGHT_KNEE].x, pose_landmarks_list[0][RIGHT_HEEL].x], [0.06, 0.05]) # 0.06, 0.06 for running
  return alignedRight
...
def initiallyLeftForward(prev_detection_result: PoseLandmarkerResult):
  ...
  return legsDetected(prev_detection_result) and pose_landmarks_list[0][LEFT_HIP].x < pose_landmarks_list[0][LEFT_FOOT_INDEX].x

def initiallyRightForward(prev_detection_result: PoseLandmarkerResult):
  pose_landmarks_list = prev_detection_result.pose_landmarks
  return legsDetected(prev_detection_result) and pose_landmarks_list[0][RIGHT_HIP].x < pose_landmarks_list[0][RIGHT_FOOT_INDEX].x
...
cap = cv2.VideoCapture("running_assets/Filtered_Running.mp4")
...
while cap.isOpened():
  ...
  if pose_frame is not None:
    ...
    if pose_result is not None:
      ...
      if (delayed_update_counter >= 3): # 3 for running
        prev_pose_result = pose_result
        delayed_update_counter = 0
